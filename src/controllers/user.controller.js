...
const updateProfile = asyncCatchError(async (req, res) => {
  const { phoneNumber, province, district, ward, add, email, fullName } = req.body;
  ...
  const user = await UserModel.findByIdAndUpdate(req.user._id, { $set: profile }, { new: true });
...
const validator = type => {
  switch (type) {
    ...
    case 'setRole':
      return [check('userId').exists(), check('role').isInt({ min: 0, max: 3 })];
    case 'changePassword':
      return [
